...
# alarm goes off!  You rush to see if you can help.  Apparently, one
# of the Elves tripped and accidentally sent the sleigh keys flying
# into the ocean!
#
...
input = lmap(int, open("01.in").read().split())

def num_increases(list):
...
# So, compare A with B, then compare B with C, then C with D, and so
...
print(
    num_increases(
        [input[i-2]+input[i-1]+input[i] for i in range(2, len(input))]
    )
)
