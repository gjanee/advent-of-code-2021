...
gamma = "".join(
    "1" if [num[p] for num in input].count("1")/len(input) > 1/2 else "0"
...
def sift(list, pos, criterion, eq_selector):
    if len(list) == 1:
        return list[0]
    sublists = [[n for n in list if n[pos] == str(v)] for v in [0, 1]]
    lens = [len(l) for l in sublists]
    if lens[0] == lens[1]:
        i = eq_selector
    else:
        i = lens.index(criterion(lens))
    return sift(sublists[i], pos+1, criterion, eq_selector)

oxygen = sift(input, 0, max, 1)
co2 = sift(input, 0, min, 0)
