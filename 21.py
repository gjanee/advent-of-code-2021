...
# ocean.  The submarine computer challenges you to a nice game of
# Dirac Dice.
#
# This game consists of a single die, two pawns, and a game board with
# a circular track containing ten spaces marked 1 through 10
...
# clockwise on spaces in order of increasing value, wrapping back
# around to 1 after 10).  So, if a player is on space 7 and they roll
# 2, 2, and 1, they would move forward 5 times, to spaces 8, 9, 10, 1,
# and finally stopping on 2.
#
# After each player moves, they increase their score by the value of
# the space their pawn stopped on.  Players' scores start at 0.  So,
# if the first player starts on space 7 and rolls a total of 5, they
# would stop on space 2 and add 2 to their score (for a total score of
# 2).  The game immediately ends as a win for any player whose score
...
#
# - Player 1 rolls 1+2+3 and moves to space 10 for a total score of
#   10.
...
# of the losing player by the number of times the die was rolled
# during the game?

from itertools import cycle, islice

p1_start, p2_start = [int(l.split()[-1]) for l in open("21.in")]

def roll_generator():
    s = cycle(range(1, 101))
    while True:
        yield sum(islice(s, 3))

def play(roll_generator, num_turns, p1, p1_score, p2, p2_score):
    # Plays player 1's turn.
    num_turns += 1
    p1 = (p1+next(roll_generator)-1)%10+1
    p1_score += p1
    if p1_score >= 1000:
        return 3*num_turns*p2_score
    else:
        return play(roll_generator, num_turns, p2, p2_score, p1, p1_score)

print(play(roll_generator(), 0, p1_start, 0, p2_start, 0))
...
# copies, one copy for each possible outcome of the die.  In this
# case, rolling the die always splits the universe into three copies:
# one where the outcome of the roll was 1, one where it was 2, and one
# where it was 3.
#
# The game is played the same as before, although to prevent things
# from getting too far out of hand, the game now ends when either
# player's score reaches at least 21.
#
...
# --------------------
#
# Straight-up dynamic programming.

from itertools import product
from collections import Counter
from functools import lru_cache

rolls = Counter(i+j+k for i, j, k in product([1, 2, 3], repeat=3))

@lru_cache(maxsize=None)
def wins(p1, p1_score, p2, p2_score):
    # Plays player 1's turn; returns the number of wins for each
    # player.
    num_p1_wins = num_p2_wins = 0
    for roll, freq in rolls.items():
        ...
        if new_p1_score >= 21:
            ...
            a, b = wins(p2, p2_score, new_p1, new_p1_score)
            num_p1_wins += b*freq
            num_p2_wins += a*freq
    return (num_p1_wins, num_p2_wins)

print(max(wins(p1_start, 0, p2_start, 0)))
