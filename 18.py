...
# the actual result of the addition operation:
# [[[[0,7],4],[[7,8],[6,0]]],[8,1]].
#
...
# times the magnitude of its left element plus 2 times the magnitude
# of its right element.  The magnitude of a regular number is just
# that number.
#
...
# --------------------
#
# Cleverly designed problem!  A tree representation is a natural fit,
# but the explode operation would be difficult (or at least, very
# messy) to implement.  We stick with strings instead.
...
def explode(s):
    # Locate a pair needing exploding.
    i = 0
    d = 0  # nesting depth
    while i < len(s):
        if s[i] == "[":
    ...
    if i == len(s):
        return s
    # Extract the pair's values and split the string into left and
    # right parts.  Notice the reversal of the left part.
    m = re.match(r"\[(\d+),(\d+)\]", s[i:])
    l, r = map(int, m.groups())
    lp, rp = s[:i][::-1], s[i+m.end():]
    # Add the right value to the right part if possible.
    m = re.search(r"\d+", rp)
    if m:
        rp = rp[:m.start()] + str(int(m.group())+r) + rp[m.end():]
    # Same for the left part (but reversed).
    m = re.search(r"\d+", lp)
    if m:
        lp = lp[:m.start()] + str(int(m.group()[::-1])+l)[::-1] + lp[m.end():]
    # Reassemble.
    return lp[::-1] + "0" + rp
...
def split(s):
    def replace(m):
        ...
        r = v-l
        return f"[{l},{r}]"
    return re.sub(r"\d{2,}", replace, s, 1)
...
def magnitude(s):
    ...
    for i in range(len(s)):
        if s[i].isdigit():  # reduced numbers have only single-digit values
...
nums = lmap(str.strip, open("18.in"))

print(magnitude(functools_reduce(add, nums)))
