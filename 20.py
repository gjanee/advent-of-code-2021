...
# mapping the floor of the ocean trench.
#
# When you get back the image from the scanners, it seems to just be
# random noise.  Perhaps you can combine an image enhancement
# algorithm and the input image (your puzzle input) to clean it up a
# little.
#
# For example:
#
...
# ..###
#
# The first section is the image enhancement algorithm.  It is
# normally given on a single line, but it has been wrapped to multiple
# lines in this example for legibility.  The second section is the
# input image, a two-dimensional grid of light pixels (#) and dark
# pixels (.).
#
...
# output image.  Each pixel of the output image is determined by
# looking at a 3x3 square of pixels centered on the corresponding
# input image pixel.  So, to determine the value of the pixel at
# (5,10) in the output image, nine pixels from the input image need to
# be considered: (4,9), (4,10), (4,11), (5,9), (5,10), (5,11), (6,9),
# (6,10), and (6,11).  These nine input pixels are combined into a
# single binary number that is used as an index in the image
...
#
# The image enhancement algorithm string is exactly 512 characters
# long, enough to match every possible 9-bit binary number.  The first
# few characters of the string (numbered starting from zero) are as
# follows:
#
...
# input image.  The small input image you have is only a small region
...
# image can also be used as an input image!  This allows it to be
# enhanced a second time:
#
# ...............
# ...............
# ...............
...
# --------------------
#
# So tricky!  Our image enhancement algorithm (iea) is such that the
# infinitely many dark pixels all turn light.  Fortunately, in the
# next step they all turn dark again.  So we alternate between
# recording light and dark pixels.

iea, p2 = open("20.in").read().split("\n\n")
assert iea[0] == "#" and iea[511] == "."
image = {
    (r, c)
    for r, l in enumerate(p2.splitlines())
    for c, p in enumerate(l)
    if p == "#"
}

def enhance(image, image_records_dark_pixels):
    # If the input image records light/dark pixels, the returned image
    # records dark/light pixels.
    new_image = set()
    target = "#" if image_records_dark_pixels else "."
    rmin, rmax = min(r for r, c in image), max(r for r, c in image)
    cmin, cmax = min(c for r, c in image), max(c for r, c in image)
    for r in range(rmin-1, rmax+2):
        for c in range(cmin-1, cmax+2):
            ...
            for nr in [r-1, r, r+1]:
                for nc in [c-1, c, c+1]:
                    v <<= 1
                    if image_records_dark_pixels^((nr, nc) in image):
                        v |= 1
            if iea[v] == target:
                new_image.add((nr, nc))
    return new_image

print(len(enhance(enhance(image, False), True)))
...
for _ in range(25):
    image = enhance(enhance(image, False), True)
print(len(image))
