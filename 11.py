...
#   an octopus to have an energy level greater than 9, it also
#   flashes.  This process continues as long as new octopuses keep
#   having their energy level increased beyond 9.  (An octopus can
...
from common import neighbors8, lmap

def load():
    return [lmap(int, l.strip()) for l in open("11.in")]
...
def step():
    ...
    while len(todo) > 0:
        r, c = todo.pop()
        for nr, nc in neighbors8(r, c, R, C):
...
grid = load()
print(len(list(takewhile(lambda _: step() < R*C, count()))) + 1)
