# --- Day 25: Sea Cucumber ---
#
# This is it: the bottom of the ocean trench, the last place the
# sleigh keys could be.  Your submarine's experimental antenna still
# isn't boosted enough to detect the keys, but they must be here.  All
...
# then you discover the phone number of a deep-sea marine biologist on
# a handwritten note taped to the wall of the submarine's cockpit.
#
...
# for-- wait, did you say submarine?  And the only place with that
# many sea cucumbers would be at the very bottom of the Mariana--"
...
# location can contain at most one sea cucumber; the remaining
...
# south-facing herd attempt to move forward one location.  When a herd
# moves forward, every sea cucumber in the herd first simultaneously
# considers whether there is a sea cucumber in the adjacent location
# it's facing (even another sea cucumber facing the same direction),
# and then every sea cucumber facing an empty location simultaneously
...
# edge.  Sea cucumbers always check whether their destination location
# is empty before moving, even if that destination is on the opposite
# side of the map:
#
# Initial state:
# ...>...
...
#
# Find somewhere safe to land your submarine.  What is the first step
# on which no sea cucumbers move?

from common import lfilter
from itertools import takewhile, count

grid = [list(l.strip()) for l in open("25.in")]
...
def step():
    ...
    for r in range(R):
        for c in lfilter(
            lambda c: grid[r][c] == ">" and grid[r][(c+1)%C] == ".",
            range(C)
        ):
            grid[r][c] = "."
            grid[r][(c+1)%C] = ">"
    ...
    for c in range(C):
        for r in lfilter(
            lambda r: grid[r][c] == "v" and grid[(r+1)%R][c] == ".",
            range(R)
        ):
            grid[r][c] = "."
            grid[(r+1)%R][c] = "v"
...
print(list(takewhile(lambda _: step() > 0, count(1)))[-1] + 1)
