...
def load():
    p1, p2 = open("13.in").read().split("\n\n")
    dots = {
        (int(x), int(y)) for x, y in [v.split(",") for v in p1.split("\n")]
    }
    folds = [
        (m[1], int(m[2]))
        for m in [
            re.match(r"fold along (x|y)=(\d+)", l) for l in p2.splitlines()
        ]
    ]
    return dots, folds
...
def fold(dots, axis, center):
    new_dots = set()
    if axis == "x":
        for x, y in dots:
            new_dots.add((x if x < center else 2*center-x, y))
    else:
        for x, y in dots:
            new_dots.add((x, y if y < center else 2*center-y))
    return new_dots
...
for axis, center in folds:
    dots = fold(dots, axis, center)
