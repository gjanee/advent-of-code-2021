...
# the cave named start, and your destination is the cave named end.
# An entry like b-d means that cave b is connected to cave d - that
...
# waste of time to visit any small cave more than once, but big caves
# are large enough that it might be worth visiting them multiple
# times.  So, all paths you find should visit small caves at most
...
# cave d and a second time when returning from cave d), and since cave
# b is small, this is not allowed.
#
...
def walk(node, counts, criterion_fn):
    if node == "end":
        return 1
    if node.islower():
        counts[node] += 1
    num_paths = sum(
        walk(n, counts, criterion_fn)
        for n in graph[node]
        if n.isupper() or criterion_fn(n, counts)
    )
    if node.islower():
        counts[node] -= 1
    return num_paths

print(walk("start", Counter(), lambda node, counts: counts[node] == 0))
...
# most twice, and the remaining small caves can be visited at most
# once.  However, the caves named start and end can only be visited
# exactly once each: once you leave the start cave, you may not return
# to it, and once you reach the end cave, the path must end
# immediately.
#
...
print(
    walk(
        "start",
        Counter(),
        lambda node, counts: counts[node] == 0
        or (node != "start" and counts.most_common(1)[0][1] == 1)
    )
)
