...
# representation into binary.  Each character of hexadecimal
...
# represents an operator that performs some calculation on one or more
...
#   the length is an 11-bit number representing the number of
#   sub-packets.
...
#   packet that contains two literal values.  This packet has a
#   version sum of 12.
...
input = (
    "".join(format(int(d, 16), "04b") for d in open("16.in").read().strip())
)

class Packet:

    def __init__(self, s, consumed=None):
        # Consumes the initial packet in s, and returns the number of
        # characters consumed in consumed[0].
        i = 0
        def consume(length):
            return int(s[i:i+length], 2), i+length
        self.version, i = consume(3)
        self.type, i = consume(3)
        if self.type == 4:
            v = ""
            flag = 1
            while flag == 1:
            ...
            self.subpackets = []
            length_type, i = consume(1)
            if length_type == 0:
                length, i = consume(15)
                n = 0
                while n < length:
                    subconsumed = [None]
                    self.subpackets.append(Packet(s[i+n:], subconsumed))
                    n += subconsumed[0]
                i += length
            else:
                num_packets, i = consume(11)
                for _ in range(num_packets):
                    subconsumed = [None]
                    self.subpackets.append(Packet(s[i:], subconsumed))
                    i += subconsumed[0]
        if consumed != None:
            consumed[0] = i
    ...
    def sum_versions(self):
        v = self.version
        if self.type != 4:
            v += sum(p.sum_versions() for p in self.subpackets)
        return v
...
def evaluate(self):
    if self.type == 4:
        return self.value
    elif self.type in [0, 1, 2, 3]:
        fns = {0: sum, 1: prod, 2: min, 3: max}
        return fns[self.type](map(Packet.evaluate, self.subpackets))
    elif self.type in [5, 6, 7]:
        fns = {5: gt, 6: lt, 7: eq}
        return (
            1 if fns[self.type](*map(Packet.evaluate, self.subpackets))
            else 0
        )
Packet.evaluate = evaluate
